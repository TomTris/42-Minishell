...
int	ft_add_rule(char ***env_o, char *rule)
{
	...
	replace = ft_strdup(rule);
	if (replace == 0)
		return (free(env_new), exit_code(1), perror("ft_strdup"), 0);
	i = -1;
	while (env_o[0][++i])
		env_new[i] = env_o[0][i];
	env_new[i] = replace;
...
int	ft_remove_rule2(char ***env_o, char **env_new)
{
	int	len;
...
int	ft_remove_rule(char ***env_o, char *rule)
{
	...
	if (env_new == 0)
		return (perror("Malloc"), exit_code(1), 0);
	env_new[len - 1] = 0;
	free(env_o[0][i]);
	env_o[0][i] = 0;
	return (ft_remove_rule2(env_o, env_new));
}

...
/*   Updated: 2024/05/22 10:43:30 by qdo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*dollar_replace2(char *str, int *i, int *cnt)
{
	...
	ret = sndup("", 0);
	if (ret == 0)
		return (exit_code(1), perror("sndup"), NULL);
	*cnt = 0;
...
char	*dollar_replace3(char *str, char **env, int *i)
{
	...
	if (temp == 0)
		return (exit_code(1), perror("Malloc failed"), free(ret), NULL);
...
char	*dollar_replace(char *str, char **env, int *i)
{
	...
	if (str[*i] == '?')
	{
		*i += 1;
		...
		if (ret == 0)
			return (exit_code(1), perror("ft_itoa"), NULL);
		return (ret);
	}
	...
	return (dollar_replace3(str, env, i));
}
